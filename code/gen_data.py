...
def gen_matrix(data='geolife'):
    if data=='ccg':
        train_data = np.load('../data/%s/real_data.npy'%data)
        gps = get_gps_ccg('../data/%s'%data)
    else:
    ...
    coords = np.column_stack((gps[0],gps[1]))
    reg2 = distance_matrix(coords, coords)
    np.fill_diagonal(reg2, 0)
